...
const menuItems = [
  ...
  { key: "categories", link: "/dashboard/products/categories" },
...
export default function MenuComponent() {
  ...
  return (
    <section className="mx-auto w-[95%] max-w-[1440px] pt-10">
      ...
      <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {menuItems.map((item) => (
          <Link
            key={item.key}
            href={item.link}
            className="group flex min-h-[156px] flex-col justify-center rounded-[12px] bg-[var(--menu-card)] px-5 py-6 text-white shadow-[0_18px_36px_rgba(17,24,39,0.08)] ring-1 ring-white/5 transition duration-200 hover:-translate-y-0.5 hover:bg-[var(--menu-card-hover)] hover:shadow-[0_22px_44px_rgba(17,24,39,0.12)]"
          >
            <span className="text-[clamp(25px,1.75vw,31px)] font-normal leading-[0.95] tracking-[-0.04em]">
              {t(`homePage.menu.${item.key}.title`)}
            </span>
            <span className="mt-3 line-clamp-2 text-[14px] leading-[1.45] text-white/82">
